...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < image.length; i++) { 
			...
			for (int j = (image[0].length - 1); j >= 0; j--) {
				flipped[i][col] = new Color(image[i][j].getRed(),  image[i][j].getGreen(), image[i][j].getBlue());
				col++;
			}
		}
		return flipped;
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int i = (image.length - 1); i >= 0; i--) { 
			for (int j = 0; j < image[0].length; j++) {
				flipped[row][j] = new Color(image[i][j].getRed(),  image[i][j].getGreen(), image[i][j].getBlue());
			}
			row++;
		}
		return flipped;
	...
	public static Color luminance(Color pixel) {
		int greyValue = (int)(0.299 * pixel.getRed() + 0.587 * pixel.getGreen() + 0.114 * pixel.getBlue());
		Color greyVersion = new Color(greyValue, greyValue, greyValue);
		return greyVersion;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < scaledImage.length; i++) {
			for (int j = 0; j < scaledImage[0].length; j++) {
				Color sourceColor = image[i * image.length / height][j * image[0].length / width];
				scaledImage[i][j] = new Color(sourceColor.getRed(), sourceColor.getGreen(), sourceColor.getBlue());
			}
		}
		return scaledImage;
	}
